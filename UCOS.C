...
void OSSched(void)
{
    ...
    if (OSLockNesting == 0 && OSIntNesting == 0) {
        ...
        OSTCBHighRdy         = OSTCBPrioTbl[p]; 
	while(OSTCBHighRdy->OSTCBStat != OS_STAT_RDY){ OSTCBHighRdy = OSTCBHighRdy->OSTCBPrioPtr;}
...
void OSTimeDly(UWORD ticks)
{
	...
	x_id = OSTCBCur->OSTCBID;
    
    
	OSTCBCur->OSTCBStat = OS_STAT_DLY;
		
	/*research the priority tasks if don't have OS_STAT_RDY,clear OSRdyTbl and OSRdyGrp*/
	tmp1 = OSTCBPrioTbl[p];
	while(tmp1 != (OS_TCB *)0 && tmp1->OSTCBStat != OS_STAT_RDY)
		tmp1 = tmp1->OSTCBPrioPtr; 
